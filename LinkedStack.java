...
public class LinkedStack<E> implements Cloneable
{
   ...
   **/ 
   ...
   /**
   * This method takes in the index from the top and returns the element at that index
   * @param n
   *  an integer that represents the index of stack where the user would like to pull information from
   * @Precondition 0 <= n and n < size( ).
   * @Postcondition The return value is the 
   *  item that is n from the top (with the top at n = 1, the next at n = 2, and so on). the stack is not changed.
   * @return The item at the index n
   **/
   public E itemAt(int n){
      ...
      if((n < 1) || (n > Node.listLength(top)))
         throw new NoSuchElementException("The index is out of the range of this linked stack: " + n);

      
      current =  Node.listPosition(top, n);
